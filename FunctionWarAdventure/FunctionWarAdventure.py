...
SCREEN_WIDTH = 17 * SIZE_WALL  # PARA QUE ENCAJE UN NUMEOR ENTERO DE PAREDES ,  (el 20 es arbitrario)
SCREEN_HEIGHT = 13 * SIZE_WALL  # PARA QUE ENCAJE UN NUMERO ENTERO DE PAREDES ( 13 ES ARBITRARIO)
MAP_HEIGHT = 13  # cuantas filas de tamaño (SCALE_TILE_SIZE) tendra de alto mi mapa
BULLET_SPEED_X = 3
BULLET_SPEED_Y = 4
CHANGE_TIME = 0.024
...
# disparo lineal
class ShootLineal(arcade.Sprite):
...
# Disparo sinoidal
class ShootSinoidal(arcade.Sprite):
...
# Disapro polinomial
class ShootPolinomial(arcade.Sprite):
...
# barrera_campana
class ShootCampana(arcade.Sprite):
...
class EnemyWorm(arcade.Sprite):
    ...
    def setup(self):
        ...
        # establece los limites de los personajes en el mapa . Fijate que su referencia es relativa
        # al mapa no a el mismo
...
class EnemyCircularSaw(arcade.Sprite):
    ...
    def setup(self):
        ...
        # Set boundaries on the left/right the enemy can't cross
        # establece los limites de los personajes en el mapa . Fijate que su referencia es relativa al mapa
...
class FinalBoss(arcade.Sprite):
    ...
    def __init__(self, filename1, filename2, scale):
        ...
        self.life = 15*LIFE  # vida de mi personaje que se reducira
    ...
    def on_update(self, delta_time: float = 1/60):
        ...
        '''Estado 1'''
        if 0.9*(LIFE*15) < self.life <= (LIFE*15):
            if self.time1 >= 0.4:
                self.disparar()
        ...
        if 0.25*(LIFE*15) < self.life <= 0.9*(LIFE*15):
            ...
            # si puede saltar pero no salta es por que esta en el suelo por tanto hacemos acciones
        ...
        if self.life <= 0.25*(LIFE*15):
            ...
            # si puede saltar pero no salta es por que esta en el suelo por tanto hacemos acciones cuando
            # esta en el suelo
    ...
    def disparar(self):
        ...
        bullet = ShootLineal("laserRed01.png", SPRITE_SCALING_LASER)
        # rotate it.
        # rotas la imagen
...
class Inicio(arcade.View):
    ...
    def __init__(self):
        ...
        # esto es para establecer las condiciones iniciales , lo pondria "on_show()" pero hay problemas
        self.setup()

        self.texture = arcade.load_texture("TitleFuncionWar.png")  # cargando mi titulo
    ...
    def set_buttons(self):
        ...
        # solo un boton agregare , fijate que el ultimo argumento se dibuja el tema
        self.button_list.append(BeginButton(self, int(SCREEN_WIDTH/2), int(SCREEN_HEIGHT*3/10),
                                            200, 100, theme=self.theme))
    ...
    def on_show(self):
        ...
        # fondo de pantalla
        arcade.set_background_color((230, 242, 223))
    ...
    def on_draw(self):
        ...
        arcade.draw_scaled_texture_rectangle(SCREEN_WIDTH/2, SCREEN_HEIGHT*3/4, self.texture, 1.8, 0)

        # arcade.draw_text("FunctionWar", SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2+200,arcade.color.BLACK, font_size=100,
        # anchor_x="center")
        arcade.draw_text("PRESIONA START PARA COMENZAR ", SCREEN_WIDTH/2, SCREEN_HEIGHT*2/5,
                         (242, 140, 56), font_size=50, anchor_x="center")
...
class Tutorial(arcade.View):
    ...
    def setup(self):
        ...
        for i in range(COIN_COUNT):
            ...
            # Position the coin
            # esos 20 es para evitar posibles bugs al momento de iniciar
            coin.center_x = random.randrange(SIZE_WALL+20, SCREEN_WIDTH-SIZE_WALL-20)
            coin.center_y = random.randrange(SIZE_WALL+20, SCREEN_HEIGHT-SIZE_WALL-20)
    ...
    # aca el jugador jugara disparando monedas , se indicara las instrucciones y cuando este listo ira a jugar en el
    # escenario principal
    def on_show(self):
    ...
    def on_draw(self):
        ...
        # Dibujamos la pantalla del tutorial
        arcade.draw_lrtb_rectangle_filled(1.2*SIZE_WALL, 4.4*SIZE_WALL, SCREEN_HEIGHT-1.1*SIZE_WALL,
                                          SCREEN_HEIGHT - 5 * SIZE_WALL, (181, 191, 126))
        arcade.draw_text("CONTROLS", 1.3*SIZE_WALL, SCREEN_HEIGHT-1.6*SIZE_WALL,
                         arcade.color.BLACK, font_size=25, anchor_x="left")
        arcade.draw_text("W: UP/JUMP \n D: RIGHT \n A: LEFT \n S: DOWN  \n 1: SHOOT LINEAL"
                         "\n 2: SHOOT SENO \n 3: SHOOT POLINOMIO \n 4: SHOOT LOG \n 5: SHOOT EXPONENCIAL "
                         "\n 6: SPECIAL TANG \n 7: BELL SHIELD \n PRESS MOUSE: SHOOT", 1.3*SIZE_WALL,
                         SCREEN_HEIGHT-5*SIZE_WALL,
                         (41, 6, 2), font_size=15, anchor_x="left")
        arcade.draw_text("PRESS ENTER TO CONTINUE", SCREEN_WIDTH - 6 * SIZE_WALL, SCREEN_HEIGHT - 1.6 * SIZE_WALL,
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if self.player_sprite.tipo_de_arma == 1:
            bullet = ShootLineal("laserBlue01.png", SPRITE_SCALING_LASER)
        elif self.player_sprite.tipo_de_arma == 2:
            bullet = ShootSinoidal("laserBlue01.png", SPRITE_SCALING_LASER)
        elif self.player_sprite.tipo_de_arma == 3:
            bullet = ShootPolinomial("laserBlue01.png", SPRITE_SCALING_LASER)
        elif self.player_sprite.tipo_de_arma == 4:
            bullet = ShootLog("laserBlue01.png", SPRITE_SCALING_LASER)
        elif self.player_sprite.tipo_de_arma == 5:
            bullet = ShootExp("laserBlue01.png", SPRITE_SCALING_LASER)
        elif self.player_sprite.tipo_de_arma == 6:
            bullet = ShootTan("laserBlue01.png", SPRITE_SCALING_LASER)
        elif self.player_sprite.tipo_de_arma == 7:
            bullet = ShootCampana("laserBlue01.png", SPRITE_SCALING_LASER)
    ...
    def update(self, delta_time):
        ...
        if self.START_GAME:
            ...
            if len(self.wall_list) == 0 and len(self.coin_list) == 0 and len(self.bullet_list) == 0:
                ...
                comenzarjuego = Juego(self.player_list, self.coin_list, self.bullet_list, self.wall_list)
                ...
                self.window.show_view(comenzarjuego)  # nose como este metodo puede funcionar
        ...
        self.bullet_list.update()
        # tenemos que actualizar en cada momento como mi personaje interacciona con la lista de paredes
...
class Juego(arcade.View):
    ...
    def __init__(self, lista_player, lista_monedas, lista_balas, lista_cajas):
        ...
        self.score = 0  # la puntuacion es lo mas importante
    ...
    def setup(self):
        ...
        for row_index in range(len(map_array)):
            ...
            for column_index in range(len(map_array[row_index])):
                ...
                item = int(map_array[row_index][column_index])  # nos aseguramos que sea entero lo que recibe
                ...
                if 0 <= item < 4:
                    ...
                    # calcula la parte superior . MAP_HEIGHT es 13 , fijalte por que es asi . NO es dificil
                    wall.top = (MAP_HEIGHT - row_index) * SIZE_WALL
                    # Add the sprite
                    self.wall_list.append(wall)  # agrega a a la lista
                elif item == 4 or item == 6 or item == 7:
                    # calacula donde estara la parte izquierda
                    enemy.left = column_index * SIZE_WALL
                    # calcula la parte superior . MAP_HEIGHT es 13 , fijalte por que es asi . NO es dificil
                    enemy.top = (MAP_HEIGHT - row_index) * SIZE_WALL
                    ...
                    # Calculate where the sprite goes
                    # calacula donde estara la parte izquierda
                    coin.center_x = (column_index+1/2) * SIZE_WALL
                    # calcula la parte superior . MAP_HEIGHT es 13 , fijalte por que es asi . NO es dificil
                    coin.center_y = (MAP_HEIGHT - row_index-1/2) * SIZE_WALL
                    # Add the sprite
    ...
    def on_draw(self):
        ...
        arcade.draw_text("RECOLECTA TODAS LAS MONEDAS PARA \n OBTENER EL DISPARO LOGARITMICO ", SIZE_WALL*6,
                         SCREEN_HEIGHT * 0.5, arcade.color.BLACK, font_size=30, anchor_x="center")
    ...
    def update(self, delta_time):
        ...
        for enemy in self.enemy_list:
            ...
            if arcade.check_for_collision(enemy, self.player_sprite) or self.player_sprite.center_y < 0:
                self.GameOver = True
                break

        for coin in self.coin_list:
            if arcade.check_for_collision(coin, self.player_sprite):
                coin.remove_from_sprite_lists()
                self.score += 1
        ...
        # mi condicion si el jugador llega a conseguir llegar aL
        # PUNTO FINAL DE ESTE MAPA QUE TIENE 153 VECES EL TAMAÑO DEL BLOQUE
        if SIZE_WALL*153 < self.player_sprite.center_x:
        ...
        if self.START_FINAL_BOSS:
            ...
            if len(self.wall_list) == 0 and len(self.coin_list) == 0 and len(self.bullet_list) == 0 \
                ...
                # pasamos self.score para que el jugador desbloquee el poder del logaritmo
                pantalla_de_carga = PantallaDeCarga(self.score)  # creamos un nuevo objeto
                # y nos vamos a la pantalla de INICIO DE JUEGO , sin llevarnos  LISTAS A LOS JUGADORES , MONEDAS
                # BALAS . DEBO LIMPIARLAS DE ALGUN MODO PARA NO TENER PROBLEMAS LUEGO CON LA MEMORAIA .
...
class FinalBattle(arcade.View):
    ...
    def __init__(self, explosiones_list, score):
        ...
        self.score = score  # la puntuacion es lo mas importante
        ...
        self.ganaste = False  # sera la condicion para que el jugador salga victorioso
        self.setup()  # es necesario hacer esto ya que el padre arcade.view no llama setup por defecto
    ...
    # aca el jugador jugara disparando monedas , se indicara las instrucciones y cuando este
    # listo ira a jugar en el escenario principal
    def on_show(self):
    ...
    def on_draw(self):
        ...
        if self.ganaste:
            arcade.draw_text("ERES UN GANADOR ", SCREEN_WIDTH / 2, SCREEN_HEIGHT * 0.7,
                             (26, 144, 217), font_size=90, anchor_x="center")
        ...
        # con este if evito bugs
        if porcentaje1 >= 0:
            barra_de_vida1("Alonso Alcala", SIZE_WALL, SIZE_WALL + 300, SCREEN_HEIGHT - 30, SCREEN_HEIGHT-SIZE_WALL,
                           arcade.color.BLACK, (0, 171, 102), porcentaje1)

        # dibujamos barra del vida del player
        # aunque la division no es entera no causa problemas en el dibujado
        porcentaje2 = self.boss_final.life / (LIFE*15)
        # manera de evitar bugs
        if porcentaje2 >= 0:
    ...
    def update(self, delta_time: float = 1/60):
        ...
        for bullet in self.bullet_list:
            ...
            if arcade.check_for_collision(self.boss_final, bullet) and self.boss_final in self.enemy_list:
                ...
                # agrega a la lista
                # hay un problema aca que hace lento cuando se inicia las explosiones
        ...
        if self.boss_final.life <= 0:
            ...
            self.boss_final.death()  # y muere lo relacionado al boss
            self.ganaste = True
            self.boss_final.life = 0  # esto es para no tener bugs en el dibujado (recuerda  la variable "porcentaje2")
        ...
        if arcade.check_for_collision_with_list(self.player_sprite, self.boss_final.bullet_list):
            ...
            if self.player_sprite.life < 0:
                self.FINAL_GAME = True
                self.player_sprite.life = 0  # para evitar bugs
...
class GameOver(arcade.View):
    ...
    def __init__(self):
        ...
        self.theme = None
        self.comenzar_tutorial = False  # la condicion que si presiona el boton Start
        self.gameover = False  # la condicion para que el juego acabe definitivamente
        # esto es para establecer las condiciones iniciales , lo pondria "on_show()" pero hay problemas si lo pongo alli
        self.setup()
        self.time = 10  # para controlar el tiempo
    ...
    def set_buttons(self):
        # self.button_list ya esta definido al llamar al padre y se encarga de guardar los botones
        # solo un boton agregare , fijate que el ultimo argumento se dibuja el tema
        self.button_list.append(BeginButton(self, SCREEN_WIDTH/2, SCREEN_HEIGHT * 3/10, 200, 100,
                                            theme=self.theme))
    ...
    def on_draw(self):
        ...
        super().on_draw()  # con esto traes todas las opciones de botones , osea el dibujado
        if self.gameover:
            arcade.draw_text("PERDISTE!!!!! ", SCREEN_WIDTH/2, SCREEN_HEIGHT * 0.7,
                             arcade.color.RED, font_size=130, anchor_x="center")
        ...
        arcade.draw_text(output, SCREEN_WIDTH/2, SCREEN_HEIGHT*0.5, arcade.color.BLACK, 100, anchor_x="center")
        # arcade.draw_text("FunctionWar", SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2+200,arcade.color.BLACK,
        # font_size=100, anchor_x="center")
        arcade.draw_text("PRESIONA EL BOTON PARA CONTINUAR ", SCREEN_WIDTH / 2, SCREEN_HEIGHT * 2 / 5,
                         (242, 140, 56), font_size=50, anchor_x="center")

    def on_update(self, delta_time: float = 1/60):

        self.time -= delta_time  # hace la cuenta del tiempo

        if self.comenzar_tutorial and self.time >= 0:
            pantalla_de_tutorial = Tutorial()  # creamos un nuevo objeto
            # y nos vamos a la pantalla de tutoriales
            self.window.show_view(pantalla_de_tutorial)  # nose como este metodo puede funcionar
        elif self.time <= 0:
            self.time = 0  # esto es para evitar operaciones como (-2)%60 que producen errores
            self.gameover = True
...
class PantallaDeCarga(arcade.View):
    ...
    def __init__(self, score):
        super().__init__()
        self.score = score  # cantidad de monedas recolectadas
        self.time = 0
    ...
    def on_draw(self):
        ...
        arcade.start_render()
        super().on_draw()  # con esto traes todas las opciones de botones , osea el dibujado
        arcade.draw_text("Cargando ....", SCREEN_WIDTH/2, SCREEN_HEIGHT / 2 + 30,
    ...
    def on_update(self, delta_time: float = 1/60):
        ...
        self.explosions_list.update()  # se actualiza las explosiones
        if self.time >= 5.3:
            batallafinal = FinalBattle(self.explosions_list, self.score)  # creamos un nuevo objeto
            # y nos vamos a la pantalla de tutoriales
...
def main():
    ...
    # cuantas monedas va a recolectar el jugador durante toda la partida para mostrarlas al final
...
1 .- Definir todas las funciones que el jugador podra disponer , Averigua como escoger una funcion que devuelva 
     funciones . Eso me vendra bine . lo hice con los propios metodos del player
...
4 .- Definir la pantalla de tutorial que a la vez son de instrucciones de juego , aca el jugador 
    escogera 3 armas(funciones de 10) . 
...
7 .- Esa plataforma estara el jefe final , este jefe final es un super_sprite que debes diseñar o buscar . 
     Tiene que tener significado con lo que inventas ojo . 
8 .- Y la pantalla de victoria , que es lo que hara el jugador , y que es lo que conseguira . 
     Que ganara como trofeo para presumirlo 
'''